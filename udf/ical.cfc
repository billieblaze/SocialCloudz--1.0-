<cfcomponent displayName="iCal" hint="A iCal processor." output="false">
	...
	<cffunction name="init" returnType="ical" access="public" output="false"
		...
		<Cfset variables.data = replace(variables.data,'\n','<BR>','all')>
		<Cfset variables.data = replace(variables.data,'\,',',','all')>
		<Cfset variables.data = replace(variables.data,'\"','"','all')>
		<cfreturn this>
	</cffunction>
	
	<cffunction name="getEvents" returnType="array" access="public" output="false"
