...
<cffunction name="uploadFromFCK" output="no" returnType="struct" access="public">
	...
	<cfif structKeyExists(form,arguments.fileFieldName) and form[fileFieldName] neq "">
		<cftry>
			...
			<cfset var filename = arguments.destination>
			...
			<cfset session.mosso.putObject( container, tempOutputFile, '#arguments.destination#/#cffile.serverfile#',this.getMimeType(cffile.serverFile)) />

				<cfset retVal.fileSize = cffile.fileSize>
				<cfset retVal.fileUrl = #containerdetails.cdn.uri# & "/" & tempOutputFile>
...
<cffunction name="uploadFile" output="no" returnType="struct" access="public">
	...
	<cfif len(x.content) gt 0>
		...
		<cfif retVal.errorCode is 0>
			<cftry>
				<cffile action="write" file="#arguments.destination#/#local.filename#" output="#x.content#" >
					<!--- cloud files --->
			<cfset var filename = local.filename>
			<cfset var extension = lcase(listLast(filename,"."))>
			<cfset var tempOutputFile = "#createUUID()#.#extension#"> 
			...
			<cfset containerDetails = session.mosso.getContainer(container) />
			<cfset session.mosso.putObject( container, tempOutputFile, '#arguments.destination#/#local.filename#', this.getMimeType(local.filename)) />
		...
		<cfif retVal.errorCode is 0>
	    	...
	    	<cfset retVal['type']= 'xhr' />
	   		...
	   		<cfset retVal['serverfilename']= tempOutputFile />
		<cfelse>
	...
	<cfelse>
		...
		<!--- cloud files --->
			...
			<cfset containerDetails = session.mosso.getContainer(container) />
			<cfset session.mosso.putObject( container, tempOutputFile, '#arguments.destination#/#local.filename#', form.filedata.content_type) />
