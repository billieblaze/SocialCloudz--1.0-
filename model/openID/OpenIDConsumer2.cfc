...
<cfcomponent name="OpenIDConsumer2" extends="OpenIDConsumer" output="false" hint="Consumer library for OpenID auth framework. Supports OpenID v1 and v2.">
	...
	<!--- OAUTH HOOK --->
		<cfset Local.OpenID['openid.ns.oauth'] = 'http://specs.openid.net/extensions/oauth/1.0'>
		<cfset Local.OpenID['openid.oauth.consumer'] = 'googlecodesamples.com'>
		<cfset Local.OpenID['openid.oauth.scope'] = 'https://www.google.com/m8/feeds/'>
		...
		<cfif Local.results['status']>
			...
			<cfset Local.openid['ns.oauth'] = "http://specs.openid.net/extensions/oauth/1.0" />
			<cfset Local.openid['oauth.consumer'] = "www.example.com" />
			<cfset Local.openid['oauth.scope='] = "http://docs.google.com/feeds/+http://spreadsheets.google.com/feeds/" />
	...
	<cffunction name="verifyAuthentication" returntype="struct" access="public" output="false" hint="Processes authentication results returned from OpenID provider.">
		...
		<cfif Variables.OpenIDSession.exists()>
			...
			<cfif not StructKeyExists(url,"openid.mode")>
			...
			<cfelseif url["openid.mode"] eq "id_res">
				...
				<cfif StructKeyExists(Local.OpenID, "nonce") and StructKeyExists(Url, "nonce") and Local.OpenID['nonce'] eq URLDecode(Url['nonce'])>
					...
					<cfif (Local.OpenID['mode'] eq "smart" and isValidSignature(Local.OpenID)) or isValidHandle(Local.OpenID)>
						<cfset Local.axResult = StructFindValue(Url, Variables.ns_ax[1]) />
						...
						<cfloop index="Local.sKey" list="#ListAppend(Local.OpenID['OpenID.sreg.required'], Local.OpenID["OpenID.sreg.optional"])#">
							...
							<cfif ArrayLen(Local.axResult) and StructKeyExists(Url, "openID.#Local.axNSalias#.value." & Local.sKey)>
								<cfset Local.result.sreg[Local.skey] = Url["openID.#Local.axNSalias#.value." & Local.skey] />
							</cfif>
						</cfloop>
						...
						<cfif ArrayLen(Local.axResult)>
							<cfloop index="Local.sKey" list="#ListAppend(Local.OpenID['openID.ax.required'], Local.OpenID['openID.ax.if_available'])#">
								<cfif StructKeyExists(Url, "openID.#Local.axNSalias#.value." & Local.sKey)>
									<cfset Local.result.ax[Local.skey] = Url["openID.#Local.axNSalias#.value." & Local.skey] />
								</cfif>
							</cfloop>
						</cfif>
	...
	<cffunction name="isValidSignature" returntype="boolean" access="private" output="false" hint="Validation of OP signature.">
		...
		<cfloop index="Local.sKey" list="#Url['openid.signed']#">
			<cfset Local.tokenContents = Local.tokenContents & "#LCase(Local.sKey)#:#Url['openid.' & Local.sKey]##chr(10)#" />
		</cfloop>
		...
		<cfif Local.Signature_base64 eq Url['openid.sig']>
			<cfreturn true />
		<cfelse>
			<cfreturn true />
		</cfif>
	...
	<cffunction name="isValidHandle" returntype="boolean" access="private" output="false" hint="Ask OP if a message is valid">
		...
		<cfhttp method="post" url="#Arguments.Request['openid_server']#" result="Local.cfhttp" proxyServer="#Variables.proxyServer#" proxyPort="#Variables.proxyPort#" proxyUser="#Variables.proxyUser#" proxyPassword="#Variables.proxyPassword#">
			...
			<cfhttpparam type="formfield" name="openid.sig" value="#UrlEncodedFormat(Url['openid.sig'])#" encoded="false" />
			<cfhttpparam type="formfield" name="openid.signed" value="#UrlEncodedFormat(Url['openid.signed'])#" encoded="false" />
			<cfloop index="Local.sKey" list="#Url['openid.signed']#">
				<cfif Local.sKey neq "mode">
					<cfhttpparam type="formfield" name="openid.#LCase(Local.sKey)#" value="#UrlEncodedFormat(Url['openid.' & Local.sKey])#" encoded="false" />
				</cfif>
			</cfloop>
		</cfhttp>
