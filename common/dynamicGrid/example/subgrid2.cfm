...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
			...
			<cfmodule template="../jqGrid.cfm" 
							...
							gridName = 'project'  <!--- Name for this grid / layout..    multiple grids can use the same grid name with different methods or arguments to reuse that gridnames layout / settings --->
							objectName = 'Project'	  <!--- Defaults to grid name.. used for friendly words like "Add Patent", "Delete Proposal" --->
							width="100%" <!--- px or % --->
							rows="10"
							
							<!--- dataURL = '/dynamicGrid/example/testAPI.cfm'   NOTE: COLMODEL MUST BE MANUALLY CONFIGURED --->
							<!--- OR  via the dynamic functionrunner --->
							method = 'application.awardproject.get'  <!--- example: application.classname.methodname  or func_method --->
							arguments = '' <!--- example: id=123&foo=bar --->
							defaultSort = 'award_project_Id' 							
							
							<!--- Subgrid --->
							subgridName = 'award'
							subgridCaption = 'Awards for this project'
							subgridMethod = 'application.award.get'
							subgridArguments = ''
							subgridKey = 'AWARD_PROJECT_ID'
							subgridValue = 'ID'
							...
							datasource='config'	
							pid='123456'
							isAdmin='0' <!--- Session variable for admin users (ie, session.admin.global)--->
			> 
	</body>
</html>