...
<!--- 	todo: this was prematurely closing out the content form! ---->
		...
		function initButtons(gridName, pager, hasSubGrid){
				<cfif attributes.showAdd eq 1>			
					$('##'+gridName).jqGrid('navButtonAdd',pager,{
						...
						onClickButton:function () {popWinDetail('#attributes.addURL#','#attributes.objectName#');},
						position: "first",
					...
					$('##'+gridName).jqGrid('navButtonAdd',pager,{
						...
						onClickButton:function () {
							...
							if (rowKey==null){
								...
								popWinDetail('#attributes.editURL#' + rowKey,'#attributes.objectName#');
							}
						},
						position: "first",
